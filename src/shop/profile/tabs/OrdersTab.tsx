...
export function OrdersTab() {
  ...
  const getStatusDisplay = (order: OrderRecord): { text: string; className: string } => {
    // If invoice exists, show invoice status
    ...
    // If payment proofs uploaded, show pending
    if (order.paymentProof1Url && order.paymentProof2Url) {
      return {
        text: 'Awaiting Approval',
        className: 'text-blue-600 font-medium',
      };
    }
    // If no proofs uploaded
    return {
  ...
  const canUploadProof = (order: OrderRecord): boolean => {
    // Can upload proof if no invoice yet and not approved
    return !order.invoiceNumber && order.paymentVerificationStatus !== 'approved';
  };
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-6">View your order history, track payment status, and download invoices.</p>
          ...
          <table className="w-full min-w-180 text-left text-sm">
            <thead>
              <tr className="border-b border-brandip-border">
                ...
                <th className="py-3 px-2 font-semibold text-brandip-heading">Invoice #</th>
                ...
                return (
                  <tr key={order.id} className="border-b border-brandip-border hover:bg-gray-50">
                    ...
                    <td className="py-3 px-2">
                      {order.invoiceNumber ? (
                        <span className="text-green-600 font-medium">{order.invoiceNumber}</span>
                      ) : (
                    ...
                    <td className={`py-3 px-2 ${status.className}`}>
                      {status.text}
                    </td>
